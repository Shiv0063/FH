...
def AddPurchaseEntry(request):
    ...
    if request.method=="POST":    
        ...
        if Terms == 'Debit':
            if PayableAmount != tamonut:
                messages.success(request,'payable Amount is no matched the Total Amount.')
    ...
    if request.method=="POST":    
        ...
        for i in stock:
            if eval(i.ProfitMargin)>0:
                al=eval(i.Quantity)*eval(i.ProfitMargin)
                ProfitMargin +=al
            else:
                al2=eval(i.Quantity)*eval(i.ProfitMargin)
                LossMargin +=al2*-1
...
def ProfitReport(request):
    Lr = SalesEntryModel.objects.filter(user=request.user,Type='Sales')
    p=0
    l=0
    for i in Lr:
